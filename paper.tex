...
\begin{figure*}
  \centering
    ...
    \begin{subfigure}{0.32\textwidth}
      ...
      \label{fig:heating-4}
    \end{subfigure}
  \mycaption{Kinetic energy (a,b) \rev{and cumulative heating (c)} as a function of time showing the development and
  measured growth rates $\gamma$ and $\lambda$ of the flute and kink
  instabilities, respectively.}{Resistivity value is $\eta=10^{-4}$
    and Figure~\ref{fig:kink_ke_log-4} is an enlarged version of
    Figure~\ref{fig:kink_ke-4}. \rev{The cumulative heating $\langle
      Q_{*} \rangle$, where $*$ is either $\nu$ for viscous heating or
      $\eta$ for Ohmic heating, is the respective heating term  
      integrated both over space and from the initial moment up to the moment $t$ in
      time. The viscous heating associated with the flute instability
      (that generated before $t\approx28$) is negligible compared to
      that associated with the kink instability (generated after
      $t\approx28$). While the isotropic 
      model permits greater viscous heating (on the order of two
      orders of magnitude), the anisotropic model enhances Ohmic
      heating.}}
...
growth of which is \rev{likely} affected by mixing of the plasma.}
...
observed displacing the axis of the tube diagonally upwards and to the right in
...
slowly as it moves the axis of the tube further through the mixed
region to eventually begin the reconnection process with the outer
region of field that is typical of the instability in this kind of
flux tube as was observed by
\revcite{\citet{quinnEffectAnisotropicViscosity2020}}.  
...
\rev{The cumulative heating $\langle
      Q_{*} \rangle$, where $*$ is either $\nu$ for viscous heating or
      $\eta$ for Ohmic heating, is the respective heating term  
      integrated both over space and from the initial moment up to the moment $t$ in
      time.         While the heat generated via viscous heating is
      orders of magnitude lower when using anisotropic viscosity,
      Ohmic heating is enhanced by the use of the anisotropic model.}} 
...
identified as the linear stage of the kink instability by inspecting the
...
$\eta$. Measurement times are listed in the main text.}  
\centering
\begin{tabular}{ccccc}
...
between the results for \rev{the two models
  of viscosity and} the two values of the resistivity. \rev{The
  theoretical average growth rate is computed as the mean across the
  radius of the ideal estimate~\eqref{eq:fluting_growth_rate2} and is
  in good agreement with the observed rate in each case, particularly
  in the less resistive case $\eta=10^{-4}$ which better represents
  ideal plasma. The
  discrepancy between predicted and observed growth rates is due,
  in part, to the growth rate estimate~\eqref{eq:fluting_growth_rate2}
  being derived under the assumption of asymptotically large values of
  $m \gg 1$, while the observed mode has a finite value of
  $m=4$. Despite this, the predicted growth rate is of a similar 
  magnitude to the observed rate. The location $r_s$ of the 
...
wavenumber of the mode with the greatest linear growth, i.e. the
mode most likely to be observed. Again, these are in good agreement
...
measured at $t=26$ in both cases. Overall, the agreement between
predicted and observed growth rates and mode wavenumbers
allows us to conclude that the observed instability is the ideal flute
instability and that expression~\eqref{eq:fluting_growth_rate2}
can be effectively applied to estimate the growth rate of the flute
instability within coronal loops.}  
...
has on the development of the flute. Our findings are discussed below.}
...
\rev{Counter-intuitively, the growth rate of the kink instability is lower in the
weakly dissipative anisotropic cases, compared to the strongly dissipative
isotropic cases as one would expect the kink instability to grow more
...
in the linear regime the flute and kink modes are able to
...
axis.} \rev{We believe that, instead of a direct coupling, the linear kink instability is
...
rates illustrated in Figures \ref{fig:heating-4}
and~\ref{fig:heating_r-3}. Anisotropic viscosity is naturally less
dissipative than isotropic 
...
in turn enhances Ohmic dissipation of heat through the generation of
strong localised current sheets. The overall effect is that more heat is generated when
...
model actually results in greater overall heating. How this finding generalises
to more realistic coronal resistivities, and whether it holds true for
other coronal instabilities, should be the subject of further investigations.}
...
and~\ref{fig:heating_r-3}) that the bulk of the viscous and Ohmic heat is
generated in the nonlinear phase of the kink instability. There is
additionally some non-negligible Ohmic heating generated prior to the
...
It is likely that the $m=4$ azimuthal mode is excited due to influences
...
eigenmode of the kink instability, resulting in the instability
growing immediately from $t=0$. In contrast, in the tubes studied here, such a perturbation must grow
from numerical noise, allowing a secondary, fluting modes perturbation to also develop
...
which this paper is based. JQ was funded via an EPSRC studentship: EPSRC DTG EP/N509668/1.

Results were obtained using the ARCHIE-WeSt High
...
can be found in the repository of ~\revcite{\citet{keith_bennett_2020_4155625}}. Associated running scripts for generating, building and running simulations on a cluster is also provided~\citet{jamie_j_quinn_2022_6327300}. The data analysis and
